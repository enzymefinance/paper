...
\end{equation}
where $p_{a_{k}}^{t_i}$ is the price per token unit of asset $a_k$ denominated in reference asset, at time $t_i$, for $k \in \mathbb{N}$.
...
The Gross Asset Value or \textit{GAV} $gav_{h_m}^{t_i}$ denominated in the reference asset of portfolio $\underline{h}_m^{t_i}$ at time $t_i$ is:
\begin{equation}
	gav_{h_m}^{t_i} = \langle \underline{p}^{t_i}, \underline{h}_m^{t_i}\rangle = \sum_{k=1}^{n} p_{a_{k}}^{t_i}h_{a_{k}}^{t_i}
\end{equation}
with the standard scalar product on $\mathbb{R}^n$.

\section{Net Asset Value}\label{app:defnav}

The Net Asset Value or \textit{NAV} $v_{h_m}^{t_i}$ denominated in the reference asset of portfolio $\underline{h}_m^{t_i}$ at time $t_i$ is:

\begin{equation}
nav_{h_m}^{t_i} = gav_{h_m}^{t_i} - r_{h_m}^{t_i}
\end{equation}

where $r_{h_m}^{t_i}$ is the rewards earned by the portfolio $m$ denominated in the reference asset of portfolio $\underline{h}_m^{t_i}$ at time $t_i$. That is in particular the $\textit{Management Fees}_{h_m}^{t_i}$ resp. $\textit{Performance Fees}_{h_m}^{t_i}$.
...
\end{eqnarray*}
Where both $I^{t_i}$ and $W^{t_i}$ is a value in denominated the reference asset. Then the Delta of a single time step is:
\begin{equation}
	\Delta_{(t_i, t_{i+1}]} = 
		\frac{v_{h_m}^{t_{i+1}} - 
...
\begin{equation}
	p_{m}^{t_i} = \Delta_{(t_0, t_i]} = \frac{nav_{h_m}^{t_i}}{\text{Total shares in existence}}
\end{equation}
